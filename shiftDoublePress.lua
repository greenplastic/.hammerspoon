...
-- how quickly must the two single shift taps occur?
module.timeFrame = 1

-- what to do when the double tap of shift occurs
...
--  flagsChanged with only shift = true
...
-- verify that *only* the shift key flag is being pressed
...
module.eventWatcher = eventtap.new({events.flagsChanged, events.keyDown}, function(ev)
    ...
    if ev:getType() == events.flagsChanged then
        if noFlags(ev) and firstDown and secondDown then -- shift up and we've seen two, so do action
            if module.action then module.action() end
            timeFirstControl, firstDown, secondDown = 0, false, false
        elseif onlyshift(ev) and not firstDown then         -- shift down and it's a first
            firstDown = true
            timeFirstControl = timer.secondsSinceEpoch()
        elseif onlyshift(ev) and firstDown then             -- shift down and it's the second
            secondDown = true
        elseif not noFlags(ev) then                        -- otherwise reset and start over
            timeFirstControl, firstDown, secondDown = 0, false, false
        end
    else -- it was a key press, so not a lone shift char -- we don't care about it
        timeFirstControl, firstDown, secondDown = 0, false, false
    end
    return false
end):start()
